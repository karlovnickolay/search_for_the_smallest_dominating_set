...
class NaiveSolver:
    def __init__(self, graph, adjacency_matrix=False):
        self.n = len(graph)
    ...
    def solve(self):
        answer, answer_mask = self.n, (1 << self.n) - 1
        ...
        while mask:
            ...
            while temp_mask:
                v = (temp_mask & -temp_mask).bit_length() - 1
                temp_mask &= temp_mask - 1

                cover_mask |= self.dominating_set[v]
                if cover_mask == (1 << self.n) - 1:
                    break

            if cover_mask == (1 << self.n) - 1:
                count_elem = mask.bit_count()

                if count_elem < answer:
                    answer = count_elem
                    answer_mask = mask
        ...
        while answer_mask:
            v = (answer_mask & -answer_mask).bit_length() - 1
